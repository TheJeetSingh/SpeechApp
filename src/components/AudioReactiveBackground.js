...
const AudioReactiveBackground = ({ audioSource = 'microphone', colorMapping = {
  lowFreq: '#1e3c72',
  midFreq: '#2a5298',
  highFreq: '#00BFFF'
}, onPermissionDenied = () => {} }) => {
  ...
  // Generate more colorful default data when no audio is available
  const generateDefaultData = () => {
    if (!isListening) {
      const time = Date.now() * 0.001;
      const defaultData = Array(128).fill(0).map((_, i) => {
  ...
  useEffect(() => {
    ...
    // Attempt to set up microphone after a short delay
  ...
  // Setup audio input source (microphone)
  const setupMicrophone = async () => {
    try {
      ...
      // Immediately update data array with current audio data
      ...
      // Call the callback to inform parent component about permission denial
      onPermissionDenied();
      // Just continue with the fallback animation
    }
  };
  ...
  // Handle canvas click to initialize audio (needed for some browsers)
  const handleCanvasClick = async () => {
    if (audioContextRef.current && audioContextRef.current.state !== 'running') {
      try {
        await audioContextRef.current.resume();
        if (!isListening && !isPermissionDenied) {
          setupMicrophone();
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const createVisualizations = () => {
      ...
      for (let i = 0; i < totalBars; i++) {
        ...
        bar.scale.y = 1 + normalizedValue * 30; // Scale the bar based on audio intensity
      ...
      for (let i = 0; i <= waveSegments; i++) {
        ...
        // Sample from audio data (wrap around if needed)
        ...
        const z = normalizedValue * 5; // Add some depth based on audio intensity
      ...
      // Pulse the glow based on average frequency intensity
      ...
      for (let i = 0; i < starCount; i++) {
        ...
        // Use audio data to determine star size if available
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: -1 }}>
      <canvas 
        ...
        title="Click to activate audio visualization"
      />
      {hasPermission && !isListening && (
        <div style={{
          ...
          Click anywhere to activate audio visualization
        </div>
      )}
    </div>
  );
